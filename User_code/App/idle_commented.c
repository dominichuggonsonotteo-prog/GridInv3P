...
/**
  * @brief  计算16位整数的绝对值
  * @param  parameter: 输入整数
  * @retval 绝对值
  */
int16_t AbsInt16(int16_t parameter)  // 计算16位整数的绝对值
{
...
/**
  * @brief  计算浮点数的绝对值
  * @param  parameter: 输入浮点数
  * @retval 绝对值
  * @note   使用CCMRAM内存加速计算
  */
CCMRAM float AbsFloat(float parameter)  // 计算浮点数的绝对值（使用CCMRAM加速）
{
...
/**
  * @brief  一阶低通滤波器实现
  * @param  output: 滤波后的输出值指针
  * @param  input: 输入值指针
  * @param  filterTs: 滤波时间常数(0.0~1.0)
  * @note   使用公式: y[n] = y[n-1]*Ts + x[n]*(1-Ts)
  *         值越大，滤波效果越强
  */
