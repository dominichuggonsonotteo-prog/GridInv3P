 ...
 * Project:      CAN Driver for STMicroelectronics STM32F3xx
 * --------------------------------------------------------------------------
...
/*! \page stm32f3_can CMSIS-Driver CAN Setup
...
CAN Mode            | <b>Master Mode</b> | <b>Master Mode</b> | <b>Master Mode</b> | n/a              | n/a
...
-----------------
  1. Under Connectivity open \b CAN Configuration:
     ...
     - <b>NVIC Settings</b>: enable interrupts
          ...
          :------------------------|:--------|:--------------------|:--------------
          CAN TX interrupts        | \b OFF  | 0                   | 0
          CAN RX0 interrupts       | \b OFF  | 0                   | 0
          CAN RX1 interrupts       | \b OFF  | 0                   | 0
          CAN SCE interrupts       | \b OFF  | 0                   | 0
...
static int32_t CAN_MessageSend (uint32_t obj_idx, ARM_CAN_MSG_INFO *msg_info, const uint8_t *data, uint8_t size) {
  ...
  if (msg_info->rtr != 0U) {                            // If send RTR requested
    ...
    ptr_CAN->sTxMailBox[obj_idx].TDHR = __UNALIGNED_UINT32_READ(data+4);
...
/**
  \fn          void CAN_TX_IRQHandler (void)
  \brief       CAN Transmit Interrupt Routine (IRQ).
*/
void CAN_TX_IRQHandler (void) {
  ...
  if (((esr & CAN_ESR_BOFF) == 0U) && ((ier & CAN_IER_BOFIE) == 0U)) { 
    ...
    if (CAN_SignalUnitEvent != NULL) { CAN_SignalUnitEvent(ARM_CAN_EVENT_UNIT_PASSIVE); }
  }
  if (((esr & CAN_ESR_EPVF) == 0U) && ((ier & CAN_IER_EPVIE) == 0U)) {
    CAN->IER |= CAN_IER_EPVIE;
    if (CAN_SignalUnitEvent != NULL) { CAN_SignalUnitEvent(ARM_CAN_EVENT_UNIT_ACTIVE); }
  }
...
/**
  \fn          void CAN_RX0_IRQHandler (void)
  \brief       CAN Receive on FIFO0 Interrupt Routine (IRQ).
*/
...
/**
  ...
  \brief       CAN Receive on FIFO1 Interrupt Routine (IRQ).
*/
...
/**
  ...
  \brief       CAN Error and Status Change Interrupt Routine (IRQ).
*/
