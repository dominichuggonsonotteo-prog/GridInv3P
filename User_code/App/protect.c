...
void ProtectPoll(void)
{
    ...
    if (g_inv3pLoop.inv3pFaultFlag != 0) {
        Inv3pPwmOff();   // 关闭PWM输出
        LED_YELLOW_ON;   // 点亮黄色指示灯
        BEEP_ON;         // 开启蜂鸣器报警
    } else {
        LED_YELLOW_OFF;  // 关闭黄色指示灯
        BEEP_OFF;        // 关闭蜂鸣器
    }
...
/**
  * @brief 快速保护检测函数
  * @param protectParamValue 被检测的参数值指针
  * @param protectValue 保护阈值
  * @param protectBit 保护标志位
  * @note 当参数值超过阈值时立即触发保护
  */
...
/**
  ...
  * @param protectParamValue 被检测的参数值指针
  * @param protectValue 保护阈值
  * @param overTime 超时计数阈值
  * @param protectBit 保护标志位
  * @param overTimeCnt 超时计数器指针
  * @note 当参数值持续超过阈值一定时间后触发保护
  */
