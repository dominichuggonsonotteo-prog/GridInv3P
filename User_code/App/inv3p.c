...
#include "inv3p.h"  // 包含逆变器控制相关的头文件

/* 全局变量定义与说明 */
PllLoopConfig g_pllLoop = {0};                   // 锁相环(PLL)控制参数结构体，初始化为零
Inv3pLoopConfig g_inv3pLoop = {0};               // 三相逆变器控制参数结构体，初始化为零
CordinateTransDef g_vacReal = {0};               // 交流电压坐标变换结构体(αβ坐标系)
CordinateTransDef g_iacReal = {0};               // 交流电流坐标变换结构体(αβ坐标系)
CordinateTransDef g_iacRef = {0};                // 交流电流参考值坐标变换结构体
CordinateTransDef g_modulateSign = {0};          // 调制信号坐标变换结构体
float g_vacRealACorrect = 0.0f;                  // A相电压校正值
float g_vacRealBCorrect = 0.0f;                  // B相电压校正值
float g_vacRealCCorrect = 0.0f;                  // C相电压校正值
float g_iacRealACorrect = 0.0f;                  // A相电流校正值
float g_iacRealBCorrect = 0.0f;                  // B相电流校正值
float g_iacRealCCorrect = 0.0f;                  // C相电流校正值
uint16_t g_inv3pLoopCnt = 0;                     // 逆变器控制循环计数器

 
/**
  * @brief  锁相环(PLL)参数初始化
  * @note   设置锁相环控制器的比例和积分参数
  */
...
/**
  * @brief  逆变器控制环参数初始化
  * @note   设置α轴和β轴电流控制器的参数，并初始化谐波控制器
  */
void Inv3pLoopParamInit(void)
{
    ...
    g_inv3pLoop.acCurrAlphaLoop.squreOfw0 = PLL_FREQ_RATED * PLL_FREQ_RATED * PI * PI * 4.0f; // 基波角频率平方(ω₀²)
    ...
    g_inv3pLoop.acCurrBetaLoop.squreOfw0 = PLL_FREQ_RATED * PLL_FREQ_RATED * PI * PI * 4.0f; // 基波角频率平方(ω₀²)
...
/**
  * @brief  PI控制器实现
  * @param  controllerOfPI: PI控制器结构体指针
  * @param  outputMax: 输出上限
  * @param  outputMin: 输出下限
  * @note   标准位置式PI控制器，包含输出限幅和状态更新
  */
...
/**
  * @brief  PR控制器实现
  * @param  controllerOfPR: PR控制器结构体指针
  * @note   谐振控制器实现，用于跟踪交流信号
  *         使用二阶广义积分器(SOGI)实现
  */
CCMRAM void PRController(PRControllerDef * controllerOfPR)
{
    ...
    sumTemp = controllerOfPR->squreOfw0 * controllerOfPR->delay1 * controllerOfPR->ts  // w0² * x1 * Ts
    ...
    controllerOfPR->delay1 += (controllerOfPR->error * PI_2 * controllerOfPR->kr  // 误差输入项：2π * Kr * e(k)
                           - (controllerOfPR->delay1 * PI_2 + sumTemp)) * controllerOfPR->ts;  // 状态反馈项：(2π * x1 + w0²*x1*Ts + x2)
